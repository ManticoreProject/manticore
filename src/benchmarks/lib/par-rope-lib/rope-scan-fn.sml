...
functor RopeScanFn (
    ...
    val reduce : ('a * 'a -> 'a) -> 'a -> 'a RTy.rope -> 'a
    val sub : 'a RTy.rope * int -> 'a
    val balanceIfNecessary : 'a RTy.rope -> 'a RTy.rope

  ) : ROPE_SCAN where type 'a rope = 'a RTy.rope = 
  struct
  ...
  (* pre: inBounds (r, i) *)
	...
	fun topdownscan K aop z rp =
	    let
		val chunks = narySplit (rp, K * RT.numAvailProcs ())
		val chunkPrefixes = listPrefix aop z (List.map (reduce aop z) chunks)
		val ps = ListPair.zip (chunkPrefixes, chunks)
	    in
		concat (RT.parMap (fn (prfx, rp) => nsscan aop prfx rp) ps)
	    end
