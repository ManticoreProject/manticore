...
functor Zipper1Fn (
  ...
  (* rope location, e.g., (rp, c), where rp is the node in focus and c is the context around the focus *)
    ...
    fun left (loc : 'a loc) : 'a loc option =
	(case loc
	  of (Leaf _, _) => NONE
	   | (Cat (_, _, l, r), c) => SOME (l, L (c, r))
	(* end case *))

    fun right (loc : 'a loc) : 'a loc option =
	(case loc
	  of (Leaf _, _) => NONE
	   | (Cat (_, _, l, r), c) => SOME (r, R (l, c))
	(* end case *))

    fun up (loc : 'a loc) : 'a loc option =
	(case loc
	  of (_, Top) => NONE
	   | (rp, L (c, r)) => SOME (append (rp, r), c)
	   | (rp, R (l, c)) => SOME (append (l, rp), c)
	(* end case*))

    fun leftmost (loc : 'a loc) : 'a loc =
	(case left loc
	  of NONE => loc
	   | SOME loc' => leftmost loc'
	(* end case *))

    fun upmost (loc : 'a loc) : 'a loc =
	(case up loc
	  of NONE => loc
	   | SOME loc' => upmost loc'
	(* end case *))

    fun upUntilLThenRight (loc : 'a loc) : 'a loc option =
	(case loc
    ...
    fun input (rp : 'a rope) : 'a loc = (rp, Top)

    fun output (loc : 'a loc) : 'a rope = 
  ...
  (* that have been visited (everything to the right in the context and "processed") *)
    fun split (unprd : 'a S.seq, prd : 'a S.seq, c : 'a ctx) : 'a rope * 'a rope =
	let fun lp (c, lr, rr) =
		(case (c, lr, rr) 
...
*)
	...
	(case c
	  of Top => 0
	   | L (c, r) => RTy.computeLength r + ctxLen c
	   | R (r, c) => RTy.computeLength r + ctxLen c)

    fun locLen (r, c) = RTy.computeLength r + ctxLen c
