...
structure GenEventLoggingPML : GENERATOR =
  ...
  (* generate the inline logging function for a given signature *)
	...
	(* generate code to copy the event arguments into the event structure *)
	  fun genPost ([], _) = ()
	    | genPost ((loc, ty)::r, i) = let
		val param = "a" ^ Int.toString i
		val items = [F.WORD loc, F.STR param]
		in
		  pr "\t    ";
		  case ty
		   of Sig.ADDR => prl["do @post-word64(log, ", param, ")\n"]
		    | Sig.INT => prl["do @post-word32(log, ", param, ")\n"]
		    | Sig.WORD => prl["do @post-word32(log, ", param, ")\n"]
		    | Sig.FLOAT => prl["do @post-word64(log, ", param, ")\n"]
		    | Sig.DOUBLE => prl["do @post-word64(log, ", param, ")\n"]
		    | Sig.EVENT_ID => prl["do @post-word64(log, ", param, ")\n"]
		    | Sig.WORD16 => prl["do @post-word16(log, ", param, ")\n"]
		    | Sig.WORD8 => prl["do @post-word8(log, ", param, ")\n"]	 
		    | Sig.NEW_ID => (
			pr "let newId : long = @NewEventId(vp)\n";
			pr "\t    ";
			pr "do @post-word64(log, newId)\n")
		    | Sig.STR n => ()
		  (* end case *);
		  genPost (r, i+1)
	    end
	  val sizes : word list = List.map (fn (_, ty) => #sz(Sig.alignAndSize ty)) args
	  val size = List.foldl (Word.+) 0w10 sizes  (*8 bytes for the timestamp and 2 bytes for the header*)
    in
	    prl ["\tdefine inline @log-event", sign, " (vp : vproc, evt : int"];
	    genParams (args, 0);
	    prl [") : ", if isSource then "long" else "()", " =\n"];
	    pr "\
	      \\t    let log : event_log = vpload(EVENT_LOG, vp)\n\
	      \";
	    prl ["\t\tdo @ensure-space(log, 0x", Word.toString size, ")\n"];
	    pr "\t\tlet evt : short = I32ToI16(evt)\n";
	    pr "\t\tdo @post-word16(log, evt)\n";
	    pr "\t\tlet time : long = ccall get_elapsed_time()\n";
	    pr "\t\tdo @post-word64(log, time)\n";
	    genPost (args, 0);
	    if isSource
