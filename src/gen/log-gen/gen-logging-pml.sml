		...
		in
		  ...
		  case ty
		   of Sig.ADDR => prf("do AdrStoreAdr((addr(any))AdrAddI32(ep, %d), %s)\n", items)
		    ...
		    | Sig.WORD16 => prf("do AdrStoreI16((addr(short))AdrAddI16(ep, %d), %s)\n", items)
		    | Sig.WORD8 => prf("do AdrStoreI8((addr(byte))AdrAddI8(ep, %d), %s)\n", items)	 
		    | Sig.NEW_ID => (
	...
	fun argToTy ({ty, ...} : Sig.arg_desc) =
	    (case ty
	      of Sig.ADDR => "'a"  (* FIXME: what about multiple ADDR args *)
	       ...
	       | Sig.EVENT_ID => "long"
	       | Sig.STR n => "string"
	       | Sig.WORD16 => raise Fail "impossible!"
	       | Sig.WORD8 => raise Fail "impossible!"
	    (* end case *))
    in
	if smallType args  (*we don't have short or byte in PML, so don't generate bindings*)
	then ()
	else( 
	prl ["    val log", name, " : "];
