...
#define READ_SET_BOUND 20
    ...
    datatype 'a vRes = Success of 'a | Failure

#define FastForward

    _primcode(
        define @mkLock(x:unit / exh:exh) : any = 
            let x : ![long] = alloc(0:long)
            let x : ![long] = promote(x)
            let x : any = (any) x
            return(x);
    )

    val mkLock : unit -> 'a = _prim(@mkLock)
    val lock = mkLock()
    fun getLock() = lock

    _primcode(

        define @getLock = getLock;
        define @lock(/exh:exh) : () =
            let lock : ![long] = @getLock(UNIT / exh)
            fun lp() : () = 
                let old : long = CAS(&0(lock), 0:long, 1:long)
                if I64Eq(old, 0)
                then
                    return()
                else
       ...
       ;
		...
		define @checkVec(v:V.vector, tv:tvar) : bool = 
			fun checkVecLoop(v:![any], i:int) : bool =
				if I32Gte(i, 0)
				then let tv' : tvar = ArrLoad(v, i)
					 if Equal(tv,tv')
				     then return(true)
					 else apply checkVecLoop(v, I32Sub(i, 1))
				else return(false)
			let x : bool = apply checkVecLoop(#0(v), #1(v))
			return(x)
		;
        ...
        define inline @fast-forward(tv : tvar, myStamp : ![stamp,int], readSet : [int, item, item], retK : cont(any), writeSet : item / exh:exh) : () = 
            ...
            fun validate(rs:item, sentinel : item, currentRS:item) : item = 
                if Equal(rs, sentinel)
                then return(Success(currentRS))
                else
                    case rs 
                       of WithK(tv':tvar, k:cont(any), ws:item,next:item,_:item) => 
                            ...
                            then
                                if I64Lt(stamp, #2(ffInfo))
                                then apply validate(next, sentinel, WithoutK(tv', currentRS)) (*we necessarily dropped this continuation, so just use a WithoutK*)
                                else return(Failure)
                            else return(Failure)
                        | WithoutK(tv':tvar, next:item) => 
                            ...
                            then
                                if I64Lt(stamp, #2(ffInfo))
                                then apply validate(next, sentinel, WithoutK(tv', currentRS)) 
                                else return(Failure)
                            else return(Failure)
                        | _ => throw exh(Fail(@"fast-forward:validate:Impossible!\n"))
                    end
            fun chkTVar(tv:tvar) : bool = 
                let stamp : stamp = #2(tv)
                       ...
                       of WithK(tv':tvar,k:cont(any),ws:List.list,next:item,nextC:item) =>
                            ...
                            then 
                                ...
                                then 
                                    ...
                                    then 
                                        BUMP_KCOUNT
                                        let valid:bool = apply chkTVar(tv')
                                        if(valid)
                                        then return(FF(WithK(tv',k,ws,#1(readSet),#2(readSet)), 1))
                                        else return(NoFF)
                                    else 
                                        ...
                                        case res
                                           of FF(currentRS:item,i:int) => 
                                                ...
                                                if (valid)
                                                then
                                                    let validateRes : vRes = apply validate(next, nextC, currentRS)
                                                    case validateRes 
                                                       of Fail => return(Done(currentRS, i))
                                                        | Success(newRS:item) => return(FF(WithK(tv', k, ws, newRS, currentRS), I32Add(i, 1)))
                                                    end
                                                else return(Done(currentRS, i))
                                            | _ => return(res)
                                        end
                                else (*continuations are not equal*)
            ...
            then return()
                ...
                let res : bool = @checkVec(#3(ffInfo), tv)
                if(res)
                then
                    let x : ffRes = apply checkFF(#0(ffInfo), #1(ffInfo))
                    case x
                       of Done(newRS:item, i:int) =>
                            case newRS
                               of WithK(tv':tvar,k:cont(any),ws:item,_:item,_:item) =>
                                    ...
                                    if I64Eq(#1(tv'), 0:long)
                                    then 
                                        if I64Lt(stamp, #0(myStamp))
                                        then 
                                            let newRS : [int,item,item] = alloc(I32Add(#0(readSet), i), newRS, newRS)
                                            do FLS.@set-key(READ_SET, newRS / exh)
                                            do FLS.@set-key(WRITE_SET, ws / exh)
                                            let captureFreq : int = FLS.@get-counter2()
                                            do FLS.@set-counter(captureFreq)
                                            do FLS.@set-key(FF_KEY, enum(0) / exh)
                                            (*do ccall M_Print_Int("Done: Fast forwarding through %d reads\n", I32Mul(i, captureFreq))*)
                                            throw k(current)
                                        else return()
                                    else return()
                               ...
                               of WithK(tv':tvar,k:cont(any),ws:item,_:item,_:item) =>
                                    ...
                                    then 
                                        ...
                                        then 
                                            ...
                                            do FLS.@set-counter(captureFreq)
                                            do FLS.@set-key(FF_KEY, enum(0) / exh)
                                            (*do ccall M_Print_Int("FF: Fast forwarding through %d reads\n", I32Mul(i, captureFreq))*)
                                            throw k(current)
                                        else return()
                                    else return()
                                | _ => let e : exn = Fail(@"fast forward: Impossible\n") throw exh(e)
