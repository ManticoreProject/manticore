...
struct
	...
	_primcode(
        ...
        typedef stampRec = ![long,int,int,long];
		...
		define @validate(readSet : RS.item, stamp : stampRec / exh:exh) : () = 
			fun validateLoopNoRec(rs : RS.item, i : int) : () = 
				case rs 
				   of RS.NilItem => 
					...
					| RS.WithoutK(tv:tvar, x:any, next:RS.item) => 
						if Equal(#0(tv), x)
		...
		define @getFullAbortNoRec(tv : tvar / exh:exh) : any = 
            let myStamp : stampRec = FLS.@get-key(STAMP_KEY / exh)
                ...
                | Option.NONE =>
                	fun getLoop() : any = 
                		...
                		then return(x)
                			...
                			do @validate(#RS_LONG_PATH(readSet), myStamp / exh)
                			apply getLoop()
        ...
        define @commit(/exh:exh) : () =
        	let readSet : RS.read_set = FLS.@get-key(READ_SET / exh)
        	let writeSet : item = FLS.@get-key(WRITE_SET / exh)
        	let stamp : stampRec = FLS.@get-key(STAMP_KEY / exh)
        	...
        	fun lockClock() : () = 
        		...
        		then return()
        			...
        			do @validate(#RS_LONG_PATH(readSet), stamp / exh)
        			apply lockClock()
        	do apply lockClock()
