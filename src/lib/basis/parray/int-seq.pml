...
val fromList = IntArray.fromList
...
fun toList v = IntArray.foldr (fn (x, ls) => x :: ls) nil v
fun singleton x = fromList (x::nil)

val tabulate = IntArray.tabulate
...
fun scan f b s = let
  ...
  val (_, ss) = IntArray.foldl f' (b, nil) s
  in
    fromListRev ss
  end
...
fun filterUntil cond f s = let
  ...
  fun null s = (IntArray.length s = 0)
