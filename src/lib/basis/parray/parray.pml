...
(*    val e = Time.now()
  ...
  fun concat (x, y) = fromRope (Rope.cat2 (toRope x, toRope y))
  fun scan rator init pa = fromRope (Rope.scan rator init (toRope pa))
  fun flatten pas = let
    val rps = Rope.map toRope (toRope pas)
    in
      fromRope (Rope.flatten rps)
    end
  fun writeBits (n, ixs) = fromRope (Rope.writeBits (n, toRope ixs))
