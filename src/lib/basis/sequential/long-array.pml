...
structure U = UnsafeLongArray
  ...
  fun tabulate (n, f : int -> long) = 
    if n < 0 then let
      val msg = "LongArray.tabulate: size " ^ Int.toString n
      in 
        Debug.failwith msg
      end
    else let
  ...
  fun map (f: long -> long) arr = tabulate (length arr, fn i => f (sub (arr, i)))
