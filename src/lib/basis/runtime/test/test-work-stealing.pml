  ...
  struct
	...
	fun fib (n : [int] / exh : PT.exh) : [int] =
		...
		return(wlit)
		 ...
		 let iv : IVar.ivar = IVar.@ivar(NONE / exh)
		 ...
		 fun bodyP (selFn : fun (PT.unit / PT.exh -> [int]) / exh : PT.exh) : [int] =
		     ...
		     return(r)           
		 fun f (unt : PT.unit / exh : PT.exh) : PT.unit =
		 ...
		 let unt : PT.unit = Cilk5WorkStealing.@push-tl(f / exh)

		 (*eval the body *)
		 let a : [int] = alloc(I32Sub(raw,1))
		 let p : [int] = apply fib(a / exh)

		 let x : PT.bool = Cilk5WorkStealing.@pop-tl(UNIT / exh)
		 do if x
		    then return()
		    else 
