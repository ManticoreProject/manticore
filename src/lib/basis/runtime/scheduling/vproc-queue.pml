...
structure VProcQueue =
  struct
     ...
     *)
      ...
      define @is-queue-geq-one (q : queue / exh : exh) : bool =
	return(Equal(q, Q_EMPTY))
      ;

      define @is-local-queue-geq-one ( / exh : exh) : bool =
	let tl : queue = vpload (VP_RDYQ_TL, host_vproc)
	let hd : queue = vpload (VP_RDYQ_HD, host_vproc)
        let b : bool = @is-queue-geq-one(tl / exh)
        if b
	   then return(b)
	else 
     ...
     *    queue0 -> rev(rest)
     *)
      define @reverse-queue (fls : FLS.fls, k : PT.fiber, rest : queue / exh : exh) : queue =
	fun revQueue (fls : FLS.fls, k : PT.fiber, rest : queue, acc : queue / exh : exh) : queue =
   ...
   (* dequeue from the secondary list *)
	...
	let vp : vproc = host_vproc
	let mask : bool = vpload(ATOMIC, vp)
	do vpstore(ATOMIC, vp, true)
	fun lp () : queue =
