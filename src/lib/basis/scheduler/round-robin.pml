...
#include "vproc-queue.def"
	...
	cont switch (s : PT.signal) =
		...
		let threadsOnLp : bool = VProcQueue.@poll-landing-pad-from-atomic (self)
                do case threadsOnLp
		    of true =>                               (* there is incoming work to do *)
		       return ()
		     | false =>                              (* there is nothing to work on *)
		       VProc.@sleep-from-atomic(self)
                end
		throw dispatch ()
	    ...
	    of PT.STOP => 
	     ...
	     | PT.PREEMPT (k : PT.fiber) =>
	       ...
	       do VProcQueue.@enqueue-from-atomic (self, fls, k)
               let _ : bool = VProcQueue.@poll-landing-pad-from-atomic (self)    (* check for incoming work *)
	       throw dispatch () 
	     | _ =>
	       let e : exn = Match
     	       throw exh(e)
