...
void M_Print_Int2(const char * s, int n1, int n2){
#ifdef NDEBUG
    printf(s, n1, n2);
#else
    const char * s2 = "[%2d] "; 
    ...
    strcpy(newStr, s2);
    strcat(newStr, s);
    printf(newStr, VProcSelf()->id, n1, n2);
#endif
}

void M_Print_Long (const char * s, int64_t n)
{
#ifdef NDEBUG
    printf(s, n);
#else
...
void PrintTime (double t, int i)
{
    char buffer [50];
    if (t < 0.001)
	printf ("VProc[%d] timer =   0.0  \n", i);
    else if (t < 1.0)
	printf ("VProc[%d] timer =   %5.3f\n", i, t);
    else if (t < 10.0)
	printf ("VProc[%d] timer =   %5.2f\n", i, t);
    else if (t < 100.0)
	printf ("VProc[%d] timer =   %5.1f\n", i, t);
    else
	printf ("VProc[%d] timer =   %3.0f\n", i, t);
}
