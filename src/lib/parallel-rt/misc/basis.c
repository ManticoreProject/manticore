...
Value_t** M_AddDequeRoots (VProc_t *vp, Value_t **rp)
{
  if (deques == NULL)
    return rp;
...
/* initialize the local deques */
void M_InitDeques (VProc_t *vp)
{
  deques = NEWVEC(ws_deque_t, NumVProcs);
...
ws_deque_t* M_GetLocalDeque (VProc_t *vp) 
{
  return &(deques[vp->id]);
}
