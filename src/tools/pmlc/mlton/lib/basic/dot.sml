...
fun escapeString s =
 ...
 | _ => Char.toString c)
       ...
       case c of
          #"\n" => "\\\\\\\\n"
        | #"\t" => "\\\\\\\\t"
        | c => concat ["\\\\\\\\", Int.fmt StringCvt.OCT (Char.ord c)]) s
...
fun optionsToString (opts: 'a list, toString: 'a -> string, sep): string =
   concat (MLtonList.separate (MLtonList.map (opts, toString), concat [sep, " "]))
...
fun labelToString (l: (string * justify) list): string =
   concat (MLtonList.concatMap (l, fn (s, j) =>
         ...
         case opt of
          ...
          | Rank (r, ns) =>
               concat ["{ ",
                       ...
                       "; ",
                       concat (MLtonList.revMap (ns, fn {nodeName} =>
                                            concat [nodeName, " "])),
                       "}"]
