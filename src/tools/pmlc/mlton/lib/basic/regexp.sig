   ...
   sig
      ...
      structure Match:
         sig
            ...
            val all: t -> (*Substring.t*)substring
            val startLength: t -> {start: int, length: int}
            val exists: t * Save.t -> bool
            val funs: t -> {exists: Save.t -> bool,
                            lookup: Save.t -> (*Substring.t*)substring,
                            peek: Save.t -> (*Substring.t*)substring option}
            val length: t -> int
            val lookup: t * Save.t -> (*Substring.t*)substring
            val lookupString: t * Save.t -> (*String.t*)string
            val peek: t * Save.t -> (*Substring.t*)substring option
            val peekString: t * Save.t -> (*String.t*)string option
            val stringFuns: t -> {exists: Save.t -> bool,
                                  lookup: Save.t -> (*String.t*)string,
                                  peek: Save.t -> (*String.t*)string option}
         end
