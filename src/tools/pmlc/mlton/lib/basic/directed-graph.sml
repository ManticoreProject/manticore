...
fun dominators (graph, {root}) =
   let
      ...
      val () = (*Int.for*)Int_for (0, numNodes, fn i =>
      ...
      fun iterate () =
         if (*Int.foldDown*)Int_foldDown (0, rootNum, false, fn (i, changed) => let
               val new =
      ...
      val () = (*Int.for*)Int_for (0, rootNum, fn i =>
      ...
      let
         ...
         val _ =
            ...
            (concat ["scc", Int.toString index], fn out =>
             Layout.outputl
             (List.layout (List.layout ((*Int.layout*)Int_layout o nodeIndex)) ns,
              out))
         val _ = destroy ()
      in
         ns
