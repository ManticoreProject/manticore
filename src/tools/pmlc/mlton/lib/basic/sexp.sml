...
fun layout sexp =
   ...
   in
      case sexp of
         Atom s => str s
       | List sexps => paren (align (MLtonList.map (sexps, layout)))
       | String s =>
            str (concat ["\"",
                         (*String.translate*)String_translate (s, fn c =>
                                           case c of
                                              #"\"" => "\"\""
                                            | #"\\" => "\\\\"
                                            | _ => (*String.fromChar*)String.str c),
                         "\""])
   end
...
fun parse (peekChar: unit -> char option,
   ...
   let
      ...
      fun atom (cs: char list): t =
         let
            fun done () = Atom ((*String.fromListRev*)String_fromListRev cs)
         in
            case peekChar () of
               NONE => done ()
             | SOME c =>
      ...
      fun string (cs: char list): t =
         case getChar () of
          ...
          | SOME c =>
               (case c of
                   #"\"" => String ((*String.fromListRev*)String_fromListRev cs)
                 | #"\\" => (case getChar () of
                                NONE => error "eof in middle of string"
                              | SOME c => string (c :: cs))
                 | _ => string (c :: cs))
