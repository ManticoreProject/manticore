...
val tableSize = (*Int.pow (2, bits)*)Word.toIntX(Word.<<(0w1, Word.fromInt bits))
...
fun hashToIndex(w: (*Word.t*)word): int = Word.toInt(Word.andb(w, mask))

fun intern(l: Element.t list, h: (*Word.t*)word) =
...
fun reset() =
   ...
   (cacheHits := 0
    ; cacheMisses := 0
    ; (*Int.for*)Int_for(0, tableSize, fn i => Array.update(table, i, Tree.new())))
...
local
   fun binary (oper: elements * elements -> elements) =
      let
         val cache = (*Array.new*)Array.array(cacheSize, NONE)
      in
         fn (s: t, s': t) =>
         let
            fun loop i =
               if i >= cacheSize
                  then
                     let
                        ...
                        val () = (*Int.inc cacheMisses*)cacheMisses := !cacheMisses + 1
                        val () =
                           Array.update (cache,
                ...
                | SOME(s1, s1', s'') =>
                     if equals(s, s1) andalso equals(s', s1')
                        then ((*Int.inc cacheHits*)cacheHits := !cacheHits + 1; s'')
                     else loop(i + 1)
         in loop 0
         end
      end
