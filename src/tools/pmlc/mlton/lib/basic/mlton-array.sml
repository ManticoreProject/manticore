...
structure MLtonArray : MLTON_ARRAY where type 'a t = 'a array =
struct
...
fun unfoldi (n, a, f) = let
      val r = ref a
      val v = Array.tabulate (n, fn i => let
		val (b, a') = f (i, !r)
		val _ = r := a'
		in
		  b
...
fun foldi2From (a, a', start, b, f) =
   ...
   in
      if n = n' andalso 0 <= start andalso start <= n 
         then loop (start, b)
      else Error.bug "Array.foldi2From"
   end
...
fun fold3From (a, a', a'', start, b, f) =
   ...
   in
      if n = n' andalso n = n'' andalso 0 <= start andalso start <= n
         then loop (start, b)
      else Error.bug "Array.fold3From"
   end
...
fun new0 () = tabulate (0, fn _ => Error.bug "Array.new0")

fun new1 x = tabulate (1, fn _ => x)

fun new2 (x0, x1) = tabulate (2, fn 0 => x0 | 1 => x1 | _ => Error.bug "Array.new2")
   ...
   let
      ...
      val _ =
         f (fn x =>
            ...
            in
               if i >= n
                  then Error.bug "Array.tabulator: too many elements"
               else (Pervasive.Array.update (a, i, SOME x)
                     ; r := i + 1)
            end)
   in
      if !r < n
         then Error.bug "Array.tabulator: not enough elements"
...
fun splitLast v =
   ...
   in
      if n <= 0
         then Error.bug "Array.splitLast"
...
val indices =
   Trace.trace ("Array.indices", layout (Layout.str o Bool.toString), layout (Layout.str o Int.toString))
   indices
...
fun removeFirst (v, f) =
   let
      ...
      val v = keepAll (v, fn a =>
                       ...
                       orelse (seen := true
                               ; false))
      val _ = if !seen then () else Error.bug "Array.removeFirst"
   in
      v
   end
...
fun shuffleN (a, n) =
   let
      val m = length a
   in
      (*Int.for*)Int_for (0, n, fn i => swap (a, i, i + Random.natLessThan (m - i)))
   end
