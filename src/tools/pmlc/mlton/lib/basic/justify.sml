...
fun spaces n = (*S.make*)String_make (n, (*C.space*)#" ")
          ...
          | SOME heads =>
               let
                  val heads = List.map2 (heads, maxs, fn (s, i) =>
                                         justify (s, i, Center))
                  val dashes = List.map (maxs, fn i => (*String.make*)String_make (i, #"-"))
               in
                  heads :: dashes :: rows
               end
   in
...
val table =
   Trace.trace ("Justify.table",
                ...
                Layout.record [("columnHeads",
                                MLtonOption.layout (List.layout (*String.layout*)Layout.str)
                                columnHeads),
                               ("justs", List.layout layout justs),
                               ("rows",
                                List.layout (List.layout (*String.layout*)Layout.str) rows)],
                List.layout (List.layout (*String.layout*)Layout.str))
   table
