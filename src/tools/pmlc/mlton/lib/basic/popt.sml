...
fun String_removeTrailing (s, pred) = Substring.string(Substring.dropr pred (Substring.full s))
...
val trace = ("trace", SpaceString (fn s =>
                                   ...
                                   in case s of
                                      "*" => always ()
                                    | _ => (flagged ()
                                            ; on ((*String.split*)String_split (s, #",")))
                                   end))
      ...
      fun loop (i, ac) =
         if i >= n
            ...
            in case c of
             ...
             | #"k" => done 1024
             | _ =>
                  case (*Char.digitToInt*)Char_digitToInt c of
                     NONE => NONE
          ...
          | switch :: switches =>
               ...
               in
                  case String.sub (switch, 0) of
                     #"-" =>
                     let val switch = (*String.dropPrefix*)String_dropPrefix (switch, 1)
                              ...
                              val rec loop' =
                                 fn [] => error (concat ["unknown switch: -",
                                  ...
                                  | (switch', arg) :: opts =>
                                       let
                                          fun doit f =
                                             if (*String.hasPrefix*)String_hasPrefix
                                                (switch, {prefix = switch'})
                                                then f ((*String.dropPrefix*)String_dropPrefix
                                       ...
                                       in case arg of
                                          Digit f =>
                                             doit (fn s =>
                                                   ...
                                                   in
                                                      if size s = 1
                                                         then (case (*Char.digitToInt*)Char_digitToInt
                                                                  (String.sub (s, 0)) of
                                                                  NONE => error ()
                                                                | SOME i => f i)
                                                      else error ()
         ...
         (fn s =>
          ...
          in
             ...
             ; List.foreach (table, fn ss =>
                             message ((*String.removeTrailing*)String_removeTrailing
