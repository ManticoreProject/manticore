...
structure Time = MLtonTime
fun String_dropPrefix (s, n) = String.substring(s, n, size s - n)
fun Date_now () = Date.fromTimeLocal(Time.now())
...
fun outputHeader (style: style, output: Layout.t -> unit) =
   let
      ...
      val lines =
         Version.banner
         :: concat ["  created this file on ", Date.toString ((*Date.now*)Date_now ()), "."]
...
fun timeToString {total, gc} =
   let
      fun fmt (x, n) = (*Real.format (x, Real.Format.fix (SOME n))*)Real.fmt (StringCvt.FIX(SOME n)) x
      val toReal = (*Real.fromIntInf*)Real.fromLargeInt o Time.toMilliseconds
      val per =
...
local
   fun msg (kind: string, r: Region.t, msg: Layout.t, extra: Layout.t): unit =
      let
         ...
         val msg =
            Layout.str
            (concat [(*String.fromChar*)String.str (Char.toUpper (String.sub (msg, 0))),
                     (*String.dropPrefix*)String_dropPrefix (msg, 1),
                     "."])
         in
...
in
   ...
   fun error (r, m, e) =
      let
         val _ = (*Int.inc numErrors*)numErrors := !numErrors + 1
