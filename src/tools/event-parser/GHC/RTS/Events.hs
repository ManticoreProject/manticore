...
manticoreParsers = [
  (FixedSizeParser 1 0 (trace "inside VProcStartIdle" (return VProcStartIdle))),
  (FixedSizeParser 3 4 (traceM "inside VProcStart" >> getE >>= \ vpId -> return $ VProcStart{vpId = vpId}) ),
  (FixedSizeParser 4 4 (traceM "inside VProcExit" >> getE >>= \ vpId -> return $ VProcExit{vpId = vpId}) ),
  (FixedSizeParser 5 0 (trace "inside VProcExitMain" (return VProcExitMain))),
  (FixedSizeParser 6 0 (trace "inside VProcIdle" (return VProcIdle))),
  (FixedSizeParser 7 0 (trace "inside VProcSleep" (return VProcSleep))),
  (FixedSizeParser 8 0 (trace "inside VProcWakeup" (return VProcWakeup))),
  (FixedSizeParser 9 4 (traceM "inside PreemptVProc" >> getE >>= \ vpid -> return $ PreemptVProc{vpid = vpid}) ),
  (FixedSizeParser 10 0 (trace "inside PreemptSignal" (return PreemptSignal))),
  (FixedSizeParser 11 8 (traceM "inside GCSignal" >> getE >>= \ sigid -> return $ GCSignal{sigid = sigid}) ),
  (FixedSizeParser 12 8 (traceM "inside MinorGCStart" >> getE >>= \ nAllocBytes -> return $ MinorGCStart{nAllocBytes = nAllocBytes}) ),
  (FixedSizeParser 13 16 (traceM "inside MinorGCEnd" >> getE >>= \ nCopiedBytes -> getE >>= \ nAvailBytes -> return $ MinorGCEnd{nAvailBytes = nAvailBytes, nCopiedBytes = nCopiedBytes}) ),
  (FixedSizeParser 16 16 (traceM "inside MajorGCStart" >> getE >>= \ nYoungBytes -> getE >>= \ nOldBytes -> return $ MajorGCStart{nOldBytes = nOldBytes, nYoungBytes = nYoungBytes}) ),
  (FixedSizeParser 17 16 (traceM "inside MajorGCEnd" >> getE >>= \ nCopiedBytes -> getE >>= \ nAvailBytes -> return $ MajorGCEnd{nAvailBytes = nAvailBytes, nCopiedBytes = nCopiedBytes}) ),
  (FixedSizeParser 20 16 (traceM "inside GlobalGCInit" >> getE >>= \ index -> getE >>= \ uid -> return $ GlobalGCInit{uid = uid, index = index}) ),
  (FixedSizeParser 21 8 (traceM "inside GlobalGCEnd" >> getE >>= \ index -> return $ GlobalGCEnd{index = index}) ),
  (FixedSizeParser 22 0 (trace "inside GlobalGCVPStart" (return GlobalGCVPStart))),
  (FixedSizeParser 23 8 (traceM "inside GlobalGCVPDone" >> getE >>= \ nbytes -> return $ GlobalGCVPDone{nbytes = nbytes}) ),
  (FixedSizeParser 24 8 (traceM "inside PromoteStart" >> getE >>= \ root -> return $ PromoteStart{root = root}) ),
  (FixedSizeParser 25 8 (traceM "inside PromoteEnd" >> getE >>= \ nbytes -> return $ PromoteEnd{nbytes = nbytes}) ),
  (FixedSizeParser 26 8 (traceM "inside ThdSpawn" >> getE >>= \ threadID -> return $ ThdSpawn{threadID = threadID}) ),
  (FixedSizeParser 27 8 (traceM "inside ThdSpawnOn" >> getE >>= \ threadID -> return $ ThdSpawnOn{threadID = threadID}) ),
  (FixedSizeParser 28 8 (traceM "inside ThdStart" >> getE >>= \ threadID -> return $ ThdStart{threadID = threadID}) ),
  (FixedSizeParser 29 0 (trace "inside ThdExit" (return ThdExit))),
  (FixedSizeParser 30 8 (traceM "inside MsgSendOffered" >> getE >>= \ msgid -> return $ MsgSendOffered{msgid = msgid}) ),
  (FixedSizeParser 31 8 (traceM "inside MsgSendResumed" >> getE >>= \ mgid -> return $ MsgSendResumed{mgid = mgid}) ),
  (FixedSizeParser 32 8 (traceM "inside MsgRecv" >> getE >>= \ msgid -> return $ MsgRecv{msgid = msgid}) ),
  (FixedSizeParser 33 8 (traceM "inside MsgRecvOffered" >> getE >>= \ opid -> return $ MsgRecvOffered{opid = opid}) ),
  (FixedSizeParser 34 16 (traceM "inside MsgRecvResumed" >> getE >>= \ msgid -> getE >>= \ opid -> return $ MsgRecvResumed{opid = opid, msgid = msgid}) ),
  (FixedSizeParser 35 16 (traceM "inside MsgSend" >> getE >>= \ msgid -> getE >>= \ recvID -> return $ MsgSend{recvID = recvID, msgid = msgid}) ),
  (FixedSizeParser 36 8 (traceM "inside WSInit" >> getE >>= \ wgid -> return $ WSInit{wgid = wgid}) ),
  (FixedSizeParser 37 8 (traceM "inside WSTerminate" >> getE >>= \ wgid -> return $ WSTerminate{wgid = wgid}) ),
  (FixedSizeParser 38 16 (traceM "inside WSWorkerInit" >> getE >>= \ wid -> getE >>= \ wgid -> return $ WSWorkerInit{wgid = wgid, wid = wid}) ),
  (FixedSizeParser 39 8 (traceM "inside WSExecute" >> getE >>= \ wid -> return $ WSExecute{wid = wid}) ),
  (FixedSizeParser 40 8 (traceM "inside WSPreempted" >> getE >>= \ wid -> return $ WSPreempted{wid = wid}) ),
  (FixedSizeParser 41 16 (traceM "inside WSThiefSend" >> getE >>= \ threadID -> getE >>= \ wid -> return $ WSThiefSend{wid = wid, threadID = threadID}) ),
  (FixedSizeParser 42 16 (traceM "inside WSThiefBegin" >> getE >>= \ threadID -> getE >>= \ wid -> return $ WSThiefBegin{wid = wid, threadID = threadID}) ),
  (FixedSizeParser 43 16 (traceM "inside WSThiefEnd" >> getE >>= \ threadID -> getE >>= \ wid -> return $ WSThiefEnd{wid = wid, threadID = threadID}) ),
  (FixedSizeParser 44 16 (traceM "inside WSThiefSuccessful" >> getE >>= \ threadID -> getE >>= \ wid -> return $ WSThiefSuccessful{wid = wid, threadID = threadID}) ),
  (FixedSizeParser 45 16 (traceM "inside WSThiefUnsuccessful" >> getE >>= \ threadID -> getE >>= \ wid -> return $ WSThiefUnsuccessful{wid = wid, threadID = threadID}) ),
  (FixedSizeParser 46 8 (traceM "inside WSSleep" >> getE >>= \ wid -> return $ WSSleep{wid = wid}) ),
  (FixedSizeParser 47 4 (traceM "inside RopeRebalanceBegin" >> getE >>= \ ropeLen -> return $ RopeRebalanceBegin{ropeLen = ropeLen}) ),
  (FixedSizeParser 48 4 (traceM "inside RopeRebalanceEnd" >> getE >>= \ ropeLen -> return $ RopeRebalanceEnd{ropeLen = ropeLen}) )]
...
-----MANTICORE EVENTS------
        VProcStartIdle  -> printf "Start idle VProc"
        VProcStart vpId -> printf "VProc %d starting" vpId
        VProcExit vpId -> printf "VProc %d exiting" vpId
        VProcExitMain  -> printf "Main vproc exiting"
        VProcIdle  -> printf "vproc going idle"
        VProcSleep  -> printf "vproc going to sleep"
        VProcWakeup  -> printf "wakeup vproc from sleep/idle"
        PreemptVProc vpid -> printf "VProc %d was preempted" vpid
        PreemptSignal  -> printf "preemption signal received"
        GCSignal sigid -> printf "global-GC signal received"
        MinorGCStart nAllocBytes -> printf "minor GC starts"
        MinorGCEnd nCopiedBytes nAvailBytes -> printf "minor GC ends"
        MajorGCStart nYoungBytes nOldBytes -> printf "major GC starts"
        MajorGCEnd nCopiedBytes nAvailBytes -> printf "major GC ends"
        GlobalGCInit index uid -> printf "global GC initiated"
        GlobalGCEnd index -> printf "global GC finished"
        GlobalGCVPStart  -> printf "global GC starts for vproc"
        GlobalGCVPDone nbytes -> printf "global GC ends for vproc"
        PromoteStart root -> printf "object promotion starts"
        PromoteEnd nbytes -> printf "object promotion ends"
        ThdSpawn threadID -> printf "local thread spawned"
        ThdSpawnOn threadID -> printf "remote thread spawned"
        ThdStart threadID -> printf "thread body starts execution"
        ThdExit  -> printf "thread exits"
        MsgSendOffered msgid -> printf "blocked message send operation"
        MsgSendResumed mgid -> printf "resumed message send operation"
        MsgRecv msgid -> printf "matching message receive operation"
        MsgRecvOffered opid -> printf "blocked message receive operation"
        MsgRecvResumed msgid opid -> printf "blocked message receive operation"
        MsgSend msgid recvID -> printf "matching message send operation"
        WSInit wgid -> printf "work-stealing group initialization"
        WSTerminate wgid -> printf "work-stealing group termination"
        WSWorkerInit wid wgid -> printf "work-stealing worker initialization"
        WSExecute wid -> printf "work stealing execution of work items"
        WSPreempted wid -> printf "work-stealing worker has been preempted"
        WSThiefSend threadID wid -> printf "work-stealing thief send"
        WSThiefBegin threadID wid -> printf "work-stealing thief begins executing on the victim vproc"
        WSThiefEnd threadID wid -> printf "work-stealing thief ends executing on the victim vproc"
        WSThiefSuccessful threadID wid -> printf "work-stealing thief successfully stole a work item"
        WSThiefUnsuccessful threadID wid -> printf "work-stealing thief failed to steal a work item"
        WSSleep wid -> printf "work-stealing worker goes to sleep"
        RopeRebalanceBegin ropeLen -> printf "A rope is being rebalanced."
        RopeRebalanceEnd ropeLen -> printf "A rope is finished being rebalanced."
...
eventTypeNum e = case e of
    VProcStartIdle -> VPROCSTARTIDLE
    Startup _  -> STARTUP
    VProcStart _  -> VPROCSTART
    VProcExit _  -> VPROCEXIT
    VProcExitMain -> VPROCEXITMAIN
    VProcIdle -> VPROCIDLE
    VProcSleep -> VPROCSLEEP
    VProcWakeup -> VPROCWAKEUP
    PreemptVProc _  -> PREEMPTVPROC
    PreemptSignal -> PREEMPTSIGNAL
    GCSignal _  -> GCSIGNAL
    MinorGCStart _  -> MINORGCSTART
    MinorGCEnd _  _  -> MINORGCEND
    StartGC -> STARTGC
    EndGC -> ENDGC
    MajorGCStart _  _  -> MAJORGCSTART
    MajorGCEnd _  _  -> MAJORGCEND
    RunThread _  -> RUNTHREAD
    StopThread _  _  -> STOPTHREAD
    GlobalGCInit _  _  -> GLOBALGCINIT
    GlobalGCEnd _  -> GLOBALGCEND
    GlobalGCVPStart -> GLOBALGCVPSTART
    GlobalGCVPDone _  -> GLOBALGCVPDONE
    PromoteStart _  -> PROMOTESTART
    PromoteEnd _  -> PROMOTEEND
    ThdSpawn _  -> THDSPAWN
    ThdSpawnOn _  -> THDSPAWNON
    ThdStart _  -> THDSTART
    ThdExit -> THDEXIT
    MsgSendOffered _  -> MSGSENDOFFERED
    MsgSendResumed _  -> MSGSENDRESUMED
    MsgRecv _  -> MSGRECV
    MsgRecvOffered _  -> MSGRECVOFFERED
    MsgRecvResumed _  _  -> MSGRECVRESUMED
    MsgSend _  _  -> MSGSEND
    WSInit _  -> WSINIT
    WSTerminate _  -> WSTERMINATE
    WSWorkerInit _  _  -> WSWORKERINIT
    WSExecute _  -> WSEXECUTE
    WSPreempted _  -> WSPREEMPTED
    WSThiefSend _  _  -> WSTHIEFSEND
    WSThiefBegin _  _  -> WSTHIEFBEGIN
    WSThiefEnd _  _  -> WSTHIEFEND
    WSThiefSuccessful _  _  -> WSTHIEFSUCCESSFUL
    WSThiefUnsuccessful _  _  -> WSTHIEFUNSUCCESSFUL
    WSSleep _  -> WSSLEEP
    RopeRebalanceBegin _  -> ROPEREBALANCEBEGIN
    RopeRebalanceEnd _  -> ROPEREBALANCEEND
    EventBlock _  _  _  -> EVENTBLOCK
...
----BEGIN MANTICORE-------
putEventSpec (VProcStartIdle ) = do
    return()
putEventSpec (VProcStart vpId) = do
    putE vpId
    return()
putEventSpec (VProcExit vpId) = do
    putE vpId
    return()
putEventSpec (VProcExitMain ) = do
    return()
putEventSpec (VProcIdle ) = do
    return()
putEventSpec (VProcSleep ) = do
    return()
putEventSpec (VProcWakeup ) = do
    return()
putEventSpec (PreemptVProc vpid) = do
    putE vpid
    return()
putEventSpec (PreemptSignal ) = do
    return()
putEventSpec (GCSignal sigid) = do
    putE sigid
    return()
putEventSpec (MinorGCStart nAllocBytes) = do
    putE nAllocBytes
    return()
putEventSpec (MinorGCEnd nCopiedBytes nAvailBytes) = do
    putE nCopiedBytes
    putE nAvailBytes
    return()
putEventSpec (MajorGCStart nYoungBytes nOldBytes) = do
    putE nYoungBytes
    putE nOldBytes
    return()
putEventSpec (MajorGCEnd nCopiedBytes nAvailBytes) = do
    putE nCopiedBytes
    putE nAvailBytes
    return()
putEventSpec (GlobalGCInit index uid) = do
    putE index
    putE uid
    return()
putEventSpec (GlobalGCEnd index) = do
    putE index
    return()
putEventSpec (GlobalGCVPStart ) = do
    return()
putEventSpec (GlobalGCVPDone nbytes) = do
    putE nbytes
    return()
putEventSpec (PromoteStart root) = do
    putE root
    return()
putEventSpec (PromoteEnd nbytes) = do
    putE nbytes
    return()
putEventSpec (ThdSpawn threadID) = do
    putE threadID
    return()
putEventSpec (ThdSpawnOn threadID) = do
    putE threadID
    return()
putEventSpec (ThdStart threadID) = do
    putE threadID
    return()
putEventSpec (ThdExit ) = do
    return()
putEventSpec (MsgSendOffered msgid) = do
    putE msgid
    return()
putEventSpec (MsgSendResumed mgid) = do
    putE mgid
    return()
putEventSpec (MsgRecv msgid) = do
    putE msgid
    return()
putEventSpec (MsgRecvOffered opid) = do
    putE opid
    return()
putEventSpec (MsgRecvResumed msgid opid) = do
    putE msgid
    putE opid
    return()
putEventSpec (MsgSend msgid recvID) = do
    putE msgid
    putE recvID
    return()
putEventSpec (WSInit wgid) = do
    putE wgid
    return()
putEventSpec (WSTerminate wgid) = do
    putE wgid
    return()
putEventSpec (WSWorkerInit wid wgid) = do
    putE wid
    putE wgid
    return()
putEventSpec (WSExecute wid) = do
    putE wid
    return()
putEventSpec (WSPreempted wid) = do
    putE wid
    return()
putEventSpec (WSThiefSend threadID wid) = do
    putE threadID
    putE wid
    return()
putEventSpec (WSThiefBegin threadID wid) = do
    putE threadID
    putE wid
    return()
putEventSpec (WSThiefEnd threadID wid) = do
    putE threadID
    putE wid
    return()
putEventSpec (WSThiefSuccessful threadID wid) = do
    putE threadID
    putE wid
    return()
putEventSpec (WSThiefUnsuccessful threadID wid) = do
    putE threadID
    putE wid
    return()
putEventSpec (WSSleep wid) = do
    putE wid
    return()
putEventSpec (RopeRebalanceBegin ropeLen) = do
    putE ropeLen
    return()
putEventSpec (RopeRebalanceEnd ropeLen) = do
    putE ropeLen
    return()
----END MANTICORE---------
