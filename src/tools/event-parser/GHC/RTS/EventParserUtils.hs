...
instance Show (EventParser a) where
  show (FixedSizeParser fsp_type fsp_size _) = "{type = " ++ show fsp_type ++ ", size = " ++ show fsp_size ++ "}"
  show (VariableSizeParser vsp_type vsp_parser) = "{variable, type = " ++ show vsp_type ++ "}"
    ...
    parser num =
        ...
        let mb_mb_et_size = do et <- M.lookup num etypes
            ...
            maybe_parser mb_et_size = do possible <- M.lookup num parser_map
                                         traceM (show (length possible) ++ " possible parsers found for " ++ show num ++ ": " ++ show possible)
            ...
            in case mb_mb_et_size of
                ...
                Just mb_et_size -> case maybe_parser mb_et_size of
                    ...
                    Nothing -> trace("Unknown Event, num = " ++ show num ++ ", size = " ++ show mb_et_size) $ noEventTypeParser num mb_et_size
...
getFixedParser size parsers =
    ...
    where pipe f g = g . f
          ...
          maybe_head [] = trace ("No parsers satisfy constraints, but " ++ show (length parsers)  ++ " exist") Nothing
