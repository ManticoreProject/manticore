...
fun mapFail(r, f) s =
   case r s of
      NONE => NONE
    | SOME(a, s) => MLtonOption.map(f a, fn b => (b, s))
...
fun stringOfLength(r, i: (*Int.t*)int) s =
   let
      fun loop(i, s, cs) =
         if i <= 0
...
fun readFromString(rm, s) =
   let val n: (*Int.t*)int = String.size s
      fun reader(i: (*Int.t*)int) =
         if i < n
      ...
      val reader =
         Trace.traceInfo
         (info,
          (*Int.layout*)Layout.str o Int.toString,
          fn NONE => Layout.str "NONE"
           | SOME(c, _) => (*Char.layout c*)Layout.str(Char.toString c),
          fn _ => (true, fn _ => true))
          reader
   in case rm reader (0: (*Int.t*)int) of
      NONE => NONE
