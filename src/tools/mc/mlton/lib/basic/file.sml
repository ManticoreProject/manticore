...
fun concat (sources, dest) =
   withOut (dest, fn out =>
           MLtonList.foreach (sources, fn f => outputContents (f, out)))

(*
val temp = MLton.TextIO.mkstemps
val tempPrefix = MLton.TextIO.tempPrefix
*)
fun tempPrefix file = OS.Path.joinDirFile {
	dir = #dir(OS.Path.splitDirFile(OS.FileSys.tmpName())),
	file = file
      }
fun temp {prefix, suffix} : string * TextIO.outstream = let
      val {file, ...} = OS.Path.splitDirFile(OS.FileSys.tmpName())
      val file = String.concat[prefix, file, suffix]
      in
	(file, TextIO.openOut file)
      end
