...
fun layout t =
   ...
   in
      case t of
       ...
       | String s => seq [str "\"", (*String.layout*)Layout.str s, str "\""]
   end
...
fun fromString (s: string): t Result.t =
   let
      ...
      fun loop (cur: int,
                ...
                state: state): int * t =
         let
            fun accum () = String ((*String.fromListRev*)String_fromListRev ac) :: prev
            fun finishChoice () =
...
val fromString =
   Trace.trace ("ChoicePattern.fromString", (*String.layout*)Layout.str, Result.layout layout)
   fromString
