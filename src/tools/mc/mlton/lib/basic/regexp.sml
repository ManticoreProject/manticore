...
local
   ...
   fun Int_forDown (start, stop, f: int -> unit): unit = let
	 fun lp i = if (i >= stop) then (f i; lp(i-1)) else ()
	 in
	   lp (start-1)
	 end
   ...
   fun Int_foldDown (start, stop, init, f : int * 'a -> 'a) : 'a = let
	 fun lp (i, acc) = if (i < start) then acc
	       else lp (i-1, f(i, acc))
	 in
	   lp (stop-1, init)
	 end
   fun Char_digitToInt c = if Char.isDigit c then SOME(ord c - ord #"0") else NONE
   fun String_alphabetize s = let
	 fun sort (l : char list) = let
	       fun loop l = (case l
		      of [] => []
		       | x :: l => let
			   fun loop' l = (case l
				  of [] => [x]
				   | x' :: l => if (x < x') then x :: x' :: l else x' :: loop' l)
			   in loop' (loop l)
			   end)
	       in loop l
	       end
	 in
	   implode (sort (explode s))
	 end
   fun String_keepAll (s, f: char -> bool) =
	 String.implode (CharVector.foldr (fn (c, ac) => if f c then c :: ac else ac) [] s)
   fun String_contains (cs, c) = Char.contains cs c
   fun Substring_substring (ss, {start, length}) = Substring.substring(ss, start, length)
   fun Substring_base ss = let val (s, start, length) = Substring.base ss
	 in
	   (s, {start = start, length = length})
	 end
   val Substring_layout = Layout.str o Substring.string
   fun Substring_endOf ss = let
	 val (_, start, length) = Substring.base ss
	 in
	   start + length
	 end
   fun Array2_tabulate (n, m, f) = Array2.tabulate Array2.RowMajor (n, m, f)
   ...
   in    
      val validChars =
         Vector.tabulate (numChars, fn i =>
                          (*String.contains*)String_contains (validCharsString, (*Char.fromInt*)Char.chr i))
            ...
            if n = numChars
               ...
               (if n >= Int.quot (numValidChars, 2)
                   then (* Character complement. *)
                      concat ["[^",
                              (*String.alphabetize*)String_alphabetize
                              ((*String.keepAll*)String_keepAll
                               (validCharsString, fn c =>
                                not ((*String.contains*)String_contains
                                     (chars, c)))),
                              "]"]
                else if (1 = String.size chars
                         andalso not ((*String.contains*)String_contains
                                      (". ", String.sub (chars, 0))))
                        then chars
                     else concat ["[", chars, "]"])
         end
      val edgeLabel =
         Trace.trace ("Regexp.edgeLabel", List.layout (*Char.layout*)(Layout.str o Char.toString), (*String.layout*)Layout.str)
         edgeLabel
   end
      ...
      struct
         ...
         fun layout (T {index, ...}) =
            let
               open Layout
            in
               seq [str "Save ", (*Int.layout*)Int_layout (!index)]
            end
         ...
         val index = Trace.trace ("Regexp.Save.index", layout, (*Int.layout*)Int_layout) index
      end
         ...
         fun layout (r: t): Layout.t =
            ...
            in
               case r of
                ...
                | CharSet f =>
                     seq [str "[",
                          str (edgeLabel ((*Int.foldDown*)Int_foldDown
                                          (0, numChars, [], fn (i, ac) =>
                                           let
                                              val c = (*Char.fromInt*)Char.chr i
                                           in
                                              if f c
                                                 then c :: ac
                                              else ac
   ...
   structure Stack:
      ...
      struct
         ...
         fun foreach (T {elts, size, ...}, f) =
            (*Int.for*)Int_for (0, !size, fn i => f (Array.sub (elts, i)))
   ...
   structure Match =
      struct
         datatype t = T of {all: (*Substring.t*)substring,
                            matches: (Save.t * (*Substring.t*)substring) array}

         fun all (T {all, ...}) = all

         val startLength = #2 o (*Substring.base*)Substring_base o all

         val endOf = (*Substring.endOf*)Substring_endOf o all

         val length = (*Substring.length*)Substring.size o all

         fun layout (T {all, matches}) =
            let open Layout
            in tuple [(*Substring.layout*)Substring_layout all,
                      Array.layout (Layout.tuple2
                                    (Save.layout, (*Substring.layout*)Substring_layout)) matches]
            end

         fun funs (T {matches, ...}) =
            let  
               fun peek (s: Save.t): (*Substring.t*)substring option =
         ...
         fun stringFuns m =
            ...
            in
               {exists = exists,
                lookup = (*Substring.toString*)Substring.string o lookup,
                peek = fn s => Option.map (peek s, (*Substring.toString*)Substring.string)}
            end
         ...
         fun peekString (m, s) = Option.map (peek (m, s), (*Substring.toString*)Substring.string)
         val lookupString = (*Substring.toString*)Substring.string o lookup
      end
   ...
   structure Actions =
      struct
         ...
         fun layout (T l) =
            List.layout (Layout.tuple2 ((*Int.layout*)Int_layout,
      ...
      struct
         ...
         datatype t =
            T of {anchorStarts: (State.t * MatchAction.t vector) vector,
                  ...
                  next: (State.t * MatchAction.t vector) array (*Array2.t*)Array2.array,
                  saves: Save.t vector,
                  seen: bool array,
      ...
      struct
                     ...
                     if numStates = 0
                        ...
                        let
                           ...
                           val c = Array.sub (charClass,
                                              (*Char.toInt*)Char.ord (String.sub (s, i)))
                           val _ =
                              Stack.foreach
                              (current, fn (s, a) =>
                               Array.foreach
         ...
         fun fromRegexp (r: Regexp.t): t =
            let
               ...
               val follow: MatchAction.t vector option (*Array2.t*)Array2.array =
                  (*Array2.new*)Array2.array (numStates, numStates, NONE)
               val posChars = (*Array2.tabulate*)Array2_tabulate (numPos, numChars, fn _ => false)
               local
               ...
               val next =
                  (*Array2.tabulate*)Array2_tabulate
                  (numStates, numChars, fn (i, c) =>
                   let
                      ...
                      val _ = (*Int.for*)Int_for (0, numStates, fn j =>
               ...
               val _ = 
                  (*Int.for*)Int_for (0, numChars, fn c =>
                           let
                              val rep = 
               ...
               val next' =
                  (*Array2.new*)Array2.array (numStates, numClasses, Array.fromList [])
               val _ =
                  List.foreach
                  (!reps, fn {char, rep} =>
                   (*Int.for*)Int_for (0, numStates, fn state =>
               ...
               val _ =
                  ...
                  (0, numStates, fn src =>
                   let
                      ...
                      val _ = 
                         (*Int.forDown*)Int_forDown
                         (0, numChars, fn c =>
                          if Vector.sub (validChars, c)
                             then
                                let
                                   val char = (*Char.fromInt*)Char.chr c
      ...
      struct
            ...
            struct
               ...
               val layout =
                  ...
                  in
                     fn Add {from, to, actions} =>
                           Layout.record
                           [("from", (*Int.layout*)Int_layout from),
                            ("to", (*Int.layout*)Int_layout to),
                            ("actions",
                             Vector.layout MatchAction.layout actions)]
                      | Init {actions, to} =>
                           Layout.record
         ...
         datatype t =
            T of {anchorStart: State.t,
                  ...
                  next: (State.t * EdgeAction.t vector) (*Array2.t*)Array2.array,
                  saves: Save.t vector,
         ...
         fun dead (numStates, numCharClasses, final, next) =
            ...
            (numStates, fn i =>
             ...
             andalso (*Int.forall*)Int_forall (0, numCharClasses, fn c =>
         ...
         fun fromNFA (nfa as NFA.T {anchorStarts, charClass,
            ...
            let
               ...
               fun visit (s: NFA.State.t): unit =
                  if Array.sub (nfaStateSave, s)
                     then ()
                        ...
                        ; (*Int.for*)Int_for (0, numCharClasses, fn c =>
                                   Array.foreach
               ...
               val _ =
                  (*Int.for*)Int_for (0, numNFAStates, fn s =>
                           if Array.sub (nfaStateSave, s)
                              then ()
                           else
                              (*Int.for*)Int_for (0, numCharClasses, fn c =>
               ...
               val next' = (*Array2.new*)Array2.array (numStates, numCharClasses,
                                       (~1, Vector.new0 ()))
         ...
         val match =
            Trace.trace ("Regexp.DFA.match",
                         fn {string, startPos, ...} =>
                         Layout.tuple [(*String.layout*)Layout.str string,
                                       (*Int.layout*)Int_layout startPos],
                         Option.layout (Layout.tuple2
                                        ((*Int.layout*)Int_layout, Actions.layout)))
            match
...
in
   structure Regexp: REGEXP =
      struct
         ...
         fun oneOf s = isChar (fn c => (*String.contains*)String_contains (s, c))
         fun notOneOf s = isNotChar (fn c => (*String.contains*)String_contains (s, c))
         ...
         fun string (s: string): t =
            seq ((*Int.foldDown*)Int_foldDown (0, String.size s, [], fn (i, ac) =>
                               char (String.sub (s, i)) :: ac))

         fun stringIgnoreCase (s: string): t =
            seq ((*Int.foldDown*)Int_foldDown
...
*)
                  ...
                  in
                     ...
                     (opt, fn (stop, Actions.T actions) =>
                      let
                              ...
                              case ma of
                                 MatchAction.Finish s =>
                                    ...
                                    in
                                       Array.update
                                       (matches, index,
                                        SOME ((*Substring.substring*)Substring_substring
                                              (string, {start = start,
                                                        length = i - start})))
                                    end
                               | MatchAction.Start s =>
                         ...
                         val all =
                            (*Substring.substring*)Substring_substring
               ...
               val match =
                  ...
                  ("Regexp.Compiled.match",
                   ...
                   Layout.record
                   [("short", (*Bool.layout*)Layout.str(Bool.toString short)),
                    ("startPos", (*Int.layout*)Int_layout startPos),
                    ("string", (*String.layout*)Layout.str string),
               ...
               fun matchAll (r, s) =
                  case matchLong (r, s, 0) of
                     NONE => NONE
                   | SOME m => if String.size s = (*Substring.length*)Substring.size (Match.all m)
                                  then SOME m
                               else NONE
         ...
         local
            ...
            and N (s: char list,
               ...
               in
                  case s of
                     d::s' => (case (*Char.digitToInt*)Char_digitToInt d of
                                  SOME d => N1 (s', d, k)
                                | NONE => raise (X "N"))
                     | _ => raise (X "N")
               end
            ...
            and A (s: char list,
               ...
               in
                  case s of 
                   ...
                   | #"\\"::(c::s) => finishC (s, c)
                   | c::s => if (*String.contains*)String_contains (")|*+?{", c)
                                then raise (X "A")
                             else finishC (s, c)
                   | _ => raise (X "A")
               end
                          ...
                          then
                             (case ces of
                                 [] =>
                                    ...
                                    (isNotChar
                                     (fn c =>
                                      cp c orelse (*String.contains*)String_contains (s, c)))
