...
structure PArrayOpGen = struct
  ...
  fun genMap (t as T.FunTy (alpha, beta)) = 
        if both FU.isInt (alpha, beta) then
	...
	else if FU.isDbl_farray alpha andalso FU.isDouble beta then
          monoVarExp' DV.map_DFF_DF
	else if FU.isDbl_farray alpha andalso FU.isDbl_farray beta then
          monoVarExp' DV.map_DFF_DFF
	else (case alpha
		 ...
		 (* else A.VarExp (DV.fpmap (), [alpha, beta]) *)
	       ...
	       else if FU.isInt t1 andalso FU.isInt_farray t2 andalso FU.isInt beta then
                 monoVarExp' DV.map_IIFF_IF
	       else if FU.isInt t1 andalso FU.isInt_farray t2 andalso FU.isInt_farray beta then
                 monoVarExp' DV.map_IIFF_IFF
