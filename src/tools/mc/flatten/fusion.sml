...
structure Fusion : sig
  ...
  fun flOp (oper : AST.fl_op) : AST.fl_op = let
    ...
    val oper' = f oper
    in
      if !changed (* confusing to C natives...this means "if it -is- changed..." *)
        then (changed := false; f oper')
        else oper'
    end

(* fuse away applications of the identity function *)
  fun fuse (e : A.exp) : A.exp = let
    val changed = ref false
    fun exp (A.LetExp (b, e)) = A.LetExp (binding b, exp e)
      ...
      | exp (A.ApplyExp (e1, e2, t)) = 
         (case exp e1
	    of A.FlOp (A.ID _) => (changed := true; exp e2)
      ...
      | exp (A.FlOp oper) = A.FlOp (flOp oper)
