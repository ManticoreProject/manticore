...
fun map (f : 'a -> 'b, xs : 'a list) : 'b list = List.map f xs
fun foldl (f : 'a * 'b -> 'b, acc : 'b, xs : 'a list) : 'b = List.foldl f acc xs
fun filter (f, xs) = List.filter f xs
...
fun fmax (x:float, y:float) = if x > y then x else y
fun fmin (x:float, y:float) = if x < y then x else y
