...
structure Future1 :> FUTURE = 
   ...
   *)
    primtype 'a future = ![Option.option('a), thunk('a), Cancelation.cancelable, FLS.fls]
		     
    primval emptyF : any = return($0)
    primval stolenF : any = return($1)
    primval evalF : any = return($2)

  (* scheduling code *)
    primtype queue = LockedQueue.queue(Process.fiber)

    primfun scheduler ( / exh : exh) : queue  = 
		     ...
		     of Process.STOP => throw dispatch ()
		      | Process.PREEMPT k =>
                ...
                (* initialize the scheduler on the vprocs. *)
                 ...
                 do apply Process.schedulerStartup (mkSwitch, parentFLS, vps / exh)
                 return (q)

    primfun eval (fut : future('a) / exh : exh) : 'a =
    ...
    primfun steal (futuresQ : queue,  fut : future('a) / exh) : () =
    ...
    primfun future (thunk : thunk('a) / exh : exh) : future('a) =
            let scheduler : fun(/exh -> queue) = scheduler   (* bind scheduler to the top-level declaration *)
    ...
    primfun touch(fut : future('a) / exh : exh) : 'a =
