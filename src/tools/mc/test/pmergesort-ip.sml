...
structure PMergesortInPlace =
  struct
   ...
   (* merge sorted arrays arr[p..q] and arr[q..r] into the sorted array dArr[p..r] *)
    ...
    fun pMergesort' (dArr, arr, p, r) = if (r-p > 1)
        then let
          val dArr' = array(alength(arr), asub(arr,p))
          val q = (p+r) div 2
          val _ = pMergesort' (dArr', arr, p, q)
          val _ = pMergesort' (dArr', arr, q, r)
          in
   ...
   (* parallel merge sort *)
	...
	in
	   pMergesort'(dArr, arr, 0, alength(arr));
	   dArr
        end
...
structure PMEx = 
  struct
    ...
    fun sortEx () = let
	val xs = [20,10000,5,3,~100,67,2,1,~12]
