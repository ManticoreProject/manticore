...
struct
  ...
  (***** controls ******)
    ...
    (*determines if this function is eligible for eta-reduction.  The body must
     *simply be a throw or apply that passes off its input argument *)
    ...
    (* apply a substitution to a variable.  If the variable
     * to be substituted has a mapping, then we recursively 
     * call this until we find the final link in the substitution
     *)
    fun subst env x = case VMap.find(env, x)
		       of SOME y =>
			  (case VMap.find(env, y)
			    of SOME z => subst env y
			     | NONE => (inc y; y))
			| NONE => x
  ...
  (* apply a substitution to a list of variables *)
    ...
    fun etaReduce(C.MODULE{name,externs,body=mainLambda as C.FB{f=main,params,rets,body=modBody}}) = 
        let fun doRHS(s, rhs) = 
		    ...
		    case lambdas
		     of nil => doExp(s', e)
		      | _ => C.mkFun(List.rev lambdas, doExp(s', e))
		end
    ...
    fun transform m =
	    ...
	    then let val _ = if !etaReduceDebug then PrintCPS.output(TextIO.openOut "eta-reduce-before.cps", m) else () 
		     val m = etaReduce m
		     val _ = if !etaReduceDebug then PrintCPS.output(TextIO.openOut "eta-reduce-after.cps", m) else ()
