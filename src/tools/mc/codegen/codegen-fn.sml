...
functor CodeGenFn (BE : BACK_END) :> CODE_GEN =
  struct
  ...
  (* Bogus binding to force inclusion of MLRisc controls *)
    ...
    fun codeGen {dst, code=M.MODULE{name, externs, code}} = let
	  ...
	  fun annotateStms ([], msg) = []
	    | annotateStms (stm :: stms, msg) = annotate(stm, msg) :: stms
	  val _ = if BE.Spec.maxGPRArgs <> List.length BE.Regs.argRegs
		then raise Fail "BE.Spec.maxGPRArgs <> List.length BE.Regs.argRegs"
		else ()
	  val _ = if BE.Spec.maxFPRArgs <> List.length BE.Regs.argFRegs
		...
		fun compareCase i = let
		    (* encode the case values according to the type of the switch value *)
		      val c = (case Var.typeOf v                      
			    ...
			    (* end case *))
		      in
			T.CMP (MTy.wordTy, T.EQ,
			  T.REG (MTy.wordTy, reg),
			  T.LI(T.I.fromInt (MTy.wordTy, Word.toIntX c)))
		      end (* compareCase *)
