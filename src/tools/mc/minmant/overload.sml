...
fun resolve () =
	...
	fun try_var (rc as (ref ov)) =
	    (case ov of
		 ...
		 let
		     fun try v::vs =
			 if U.unifiable (ty, Var.typeOf v)
			 then try vs
			 else ((case vs of
				    [] => raise Fail "type mismatch"
				  | [x] => (U.unify (ty, Var.typeOf x); rc := AST.Instance x)
	...
	fun set_var_defaults () =
		...
		fun set_def (ref ov) =
		    (case ov of
			 AST.Unknown (ty, _) => set_def_ty (TypeUtil.pruneTy ty)
		       | AST.Instance _ => ()
		    (* end case *))
	    in
	...
	fun set_var_defaults () = List.app (fn (ref (ty, _)) => set_def_ty ty) !lits
