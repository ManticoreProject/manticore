 ...
 * All rights reserved.
 *
 * Utility definitions used by both the HLOp and HLRW languages.
 *)

(* FIXME: Maybe this should be some kind of functor; however, this
   might be difficult since the tokens are different - maybe a functor
   over token constructors? *)

structure HLDefUtils = struct

  (* Curried means to make a FLOAT token from a substring *)
    fun mkMkFloat tokCon = fn ss => let
