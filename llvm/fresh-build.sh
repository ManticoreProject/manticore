...
# OPTIONS=""
# and change the build generator from "Ninja" to "Unix Makefiles"
...
mkdir build install
pushd build
cmake -G "Ninja" -DCMAKE_INSTALL_PREFIX=../install -DCMAKE_BUILD_TYPE=Release $OPTIONS -DLLVM_TARGETS_TO_BUILD="X86" ../src
make -j $JOBS opt llc
./bin/opt --version
./bin/llc --version
