...
\paragraph{Interrupt}
The Interrupt operation (\figref{fig:protocol1-interrupt}) zeros the vproc's limit pointer after performing
a Send.
Subsequently, at the next heap-limit test, the vproc is guaranteed to stop and handle its pending signals,
including the one we just sent.
...
\paragraph{Interrupt}
The Interrupt operation (\figref{fig:protocol2-interrupt}) is identical to the one given
for protocol 1.
...
void StartGlobalGC (VProc_t *self, Value_t **roots)
{
  ...
  if (leaderVProc) {
    for (int i = 0; i < nVProcs; i++)
      if (VProcs[i] != self)
        Interrupt(VProcs[i], dummyCont, M_NIL);
    MutexLock(&GCLock);
